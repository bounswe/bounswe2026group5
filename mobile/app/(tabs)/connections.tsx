...
function MentorConnections() {
  ...
  const requests = (requestsQuery.data ?? []) as ApiRequest[];
  const matches = (matchesQuery.data ?? []) as ApiMatch[];
  const requestsLoading = requestsQuery.isLoading;
  const matchesLoading = matchesQuery.isLoading;
  const requestsError = Boolean(requestsQuery.error);
  const matchesError = Boolean(matchesQuery.error);
  ...
  const handleDeclineConfirmed = () => {
    if (declineTargetId) {
      respondMutation.mutate(
        { requestId: declineTargetId, action: "reject" },
        {
          onError: (error: Error) => {
            Alert.alert(
              "Action Failed",
