...
export function AvailabilityPreview({ schedule = [], onEdit }: AvailabilityPreviewProps) {
  const [expandedDay, setExpandedDay] = useState<string | null>(null);

  const currentSlots = expandedDay ? (schedule?.find(s => s.day === expandedDay)?.times || []) : [];
      ...
      <View>
        ...
        >
          {WEEK_DAYS.map((day, index) => {
            ...
            const hasSlots = schedule?.some(s => s.day === day && s?.times?.length > 0);
            ...
            return (
              ...
              >
                ...
                <View className={`w-1 h-1 rounded-full mt-1.5 ${
                  hasSlots ? (isExpanded ? 'bg-white' : 'bg-blue-600') : 'bg-transparent'
                }`} />
              </TouchableOpacity>
            );
          })}
        ...
        <View className="bg-gray-50 p-4 rounded-2xl border border-gray-100 mt-4 min-h-[120px] justify-center animate-fade-in">
          {currentSlots.length === 0 ? (
            <View className="items-center py-4">
              <Ionicons name="calendar-clear-outline" size={32} color="#d1d5db" className="mb-2" />
              <Text className="text-gray-500 font-medium">No availability on {expandedDay}</Text>
            </View>
          ) : (
