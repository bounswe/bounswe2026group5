...
const PASSWORD = 'AvailabilityPass123!';
...
export class TestDataApi {
  ...
  async fetchMyMatches(auth: AuthResponse) {
    ...
    expect(response.ok()).toBeTruthy();
    return response.json() as Promise<Array<{ mentor: { username: string }; mentee: { username: string }; is_active: boolean }>>;
  }
  ...
  async fetchMySessions(auth: AuthResponse, query = '?role=mentee&status=upcoming') {
    ...
    expect(response.ok()).toBeTruthy();
    return response.json() as Promise<Array<{ mentor: { username: string }; mentee: { username: string }; display_status: string }>>;
  }
